...
namespace RulesForOperationProceeding.Data.Repositories
{
    public class OperationParameterRepository:BaseRepository, IOperationParameterRepositor
    {
        ...
        public IAsyncEnumerable<OperationParameterModel> GetOperationParametersById(Guid operationyTypeId) => _context.OperationParameters.Where(x => x.OperationTypeId == operationyTypeId).AsAsyncEnumerable();
        public async Task<OperationParameterModel> GetOperationParameter(Guid operationParameterId) => await _context.OperationParameters.Where(x => x.Id == operationParameterId).FirstOrDefaultAsync();
